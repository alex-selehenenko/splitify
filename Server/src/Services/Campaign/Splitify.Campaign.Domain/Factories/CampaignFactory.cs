...
namespace Splitify.Campaign.Domain.Factories
{
    ...
    {
        public static Result<CampaignAggregate> Create(string id, string? userId, List<Link> links, IDateTimeService dateTimeService)
        {
            var validationResult = ValidateId(id)
                .Then(res => ValidateLinks(res, links));

            return validationResult.IsSuccess
                ? Result.Success(new CampaignAggregate(id, userId, dateTimeService.UtcNow, links))
